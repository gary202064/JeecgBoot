...
CREATE TABLE IF NOT EXISTS `hn_monthly_record` (
  ...
  `production_order_no` varchar(64) DEFAULT NULL COMMENT '生产订单编号',
  `document_date` date DEFAULT NULL COMMENT '单据日期',
  `batch_no` varchar(64) DEFAULT NULL COMMENT '批号',
  `scrap_qty` int DEFAULT NULL COMMENT '工废数量',
  `material_waste_qty` int DEFAULT NULL COMMENT '料费数量',
  `defect_qty` int DEFAULT NULL COMMENT '次品数量',
  `inspector` varchar(64) DEFAULT NULL COMMENT '检验员',
  `wheel_waste_qty` int DEFAULT NULL COMMENT '轮废数量',
  `rod_waste_qty` int DEFAULT NULL COMMENT '杆废',
